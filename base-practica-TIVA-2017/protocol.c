...
/* Envia la trama usando la funcion bloqueante xSerialWrite ---> Esta funcion es bloqueante  y no se puede llamar desde una ISR */

int32_t send_frame(uint8_t *frame, int32_t FrameSize)
{
	FrameSize=xUSBSerialWrite(frame,FrameSize,portMAX_DELAY);
	return FrameSize;	
}
